...
ALLOWED_IMPORT = {"admin", "data_scientist", "staff", "physio", "performance_coach"}  # wie mag import/export/matches beheren
...
def df_to_db_rows(df: pd.DataFrame, source_file: str, name_to_id: dict) -> tuple[list[dict], list[str]]:
    ...
    for idx, r in df.iterrows():
        ...
        base = {
            ...
            "type": t,
            "event": ev,
            "match_id": None,  # ✅ always auto later
            "source_file": source_file,
        }
...
# ---- UI security: spelers mogen deze pagina NIET zien ----
# (DB/RLS blijft leidend; dit is alleen UI-gate)
role_ui = (role or st.session_state.get("role") or "").strip().lower()
if role_ui == "player":
    st.error("Geen toegang.")
    st.stop()

# optioneel: sla role op voor andere pagina's
...
if main_page == "Import GPS":
    ...
    # -------------------------
    # SUB: Manual add (tabel) — UX verbeterd + metric selector
    # -------------------------
